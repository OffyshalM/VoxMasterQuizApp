...
let hideButtons = document.getElementById("screenReaderBtn");
...
function speakQuestion() {
    ...
    let optionText = currentQuestion.options;

    let speechOutput = `Question ${questionNumber}: ${questionText}.`;

    for (let i = 0; i < optionText.length; i++) {
        speechOutput += ` Option ${optionLabel[i]}: ${optionText[i]}.`;
    }

    utterance = new SpeechSynthesisUtterance(speechOutput);
...
showQuestion = (function (originalFn) {
    return function () {
...
function initializeQuizAfterVoicesLoad() {
